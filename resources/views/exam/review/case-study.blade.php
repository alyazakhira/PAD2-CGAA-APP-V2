            ...
            <div class="d-flex flex-column bg-blue-dark1 p-4 vh-100 justify-content-between align-items-center quest-sidebar" style="overflow-y: auto; top: 0;">
                ...
                <div class="d-flex flex-column">
                    ...
                    <div class="d-grid" style="grid-template-columns: 1fr 1fr 1fr 1fr; width: fit-content; gap: 20px">
                        <a href="{{ route('exam.review.cs', $session_id) }}#information" type="button" class="btn btn-blue-light text-center number" style="width: 40px; height: 40px;">
                            <i class="bi bi-info-lg"></i>
                        </a>
                        @for ($i = 1; $i <= $content->instruction_count; $i++)
                            @if ($answer->{"answer_$i"})
                                <a href="{{ route('exam.review.cs', $session_id) }}#{{ $i }}" type="button" class="btn btn-blue-normal text-center number" style="width: 40px; height: 40px;">
                                    {{ $i }}
                                </a>
                            @else
                                <a href="{{ route('exam.review.cs', $session_id) }}#{{ $i }}" type="button" class="btn btn-blue-light text-center number" style="width: 40px; height: 40px;">
                                    {{ $i }}
                                </a>
                            @endif
                        @endfor
